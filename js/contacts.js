    ...
    $(document).on('click', '.contacts-addr-item.addr-item', function () {
        ...
        for (let value of myCollection) {
            if (city_id == value.properties.get('key_my')){
                ...
                $('.dg-link').attr('href', 'https://2gis.ru/kazan/search/' + value.properties.get('addr') + ',' + 'Казань')
                $('.gg-link').attr('href', 'https://google.ru/maps/place/' + value.properties.get('addr') + ',' + 'Казань')
                $('.ym-link').attr('href', 'https://yandex.ru/maps/43/kazan/?mode=search&source=morda&text='+ value.properties.get('addr') + ',' + 'Казань')
